...
public class Compiler {
	...
	/**
	 * 수식 문자열을 컴파일하는 메소드 1. 문자열을 토큰(숫자/연산자)로 분리 2. 중위표현식을 후위표현식으로 변환 (연산자 우선순위 처리)
	...
	public int compile(String inputString) {
	    ...
	    bus.compToReg("load " + commandFindIndex-- , memory[commandCount++]);

		// 후위표기법 수식을 순회하며 명령어/데이터를 메모리에 저장
		for (String token : postfix) {
			if (isNumber(token)) {
				// 숫자이면 메모리 뒤쪽부터 저장
				bus.compToReg(token, memory[operandCount--]);
			} else {
				// 연산자이면 메모리 앞쪽부터 저장
				switch (token) {
				case "+":
					bus.compToReg("add " + commandFindIndex-- , memory[commandCount++]);
					break;
				case "-":
					bus.compToReg("sub " + commandFindIndex-- , memory[commandCount++]);
					break;
				case "*":
					bus.compToReg("mul " + commandFindIndex-- , memory[commandCount++]);
					break;
				case "/":
					bus.compToReg("div " + commandFindIndex-- , memory[commandCount++]);
					break;
				case "%":
					bus.compToReg("mod " + commandFindIndex-- , memory[commandCount++]);
					break;
				default:
					break;
				} // Switch_end
		...
		// 종료 : 마지막으로 메모리에 저장
	    bus.compToReg("store " + commandFindIndex , memory[commandCount++]);
