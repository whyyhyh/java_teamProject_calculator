...
public class Bus {
    ...
    private String tempararySender;
    private String tempararyAccepter;
 ...
 // 공통 대기 메시지
    public void waitForEnter(String stepDescription) {
        tui.displayStructure(tempararySender, tempararyAccepter);
     ...
     * sender → accepter
     * 레지스터 간에 값을 전달합니다. (Register to Register)
     * 데이터 버스의 역할을 시뮬레이션합니다.
     *
    ...
    public void regToReg(Register sender, Register accepter) {
        ...
        // 데이터 이동의 두 주체의 이름을 필드에 저장
        tempararySender = sender.getClass().getSimpleName();
        tempararyAccepter = accepter.getClass().getSimpleName();
        
        waitForEnter(tempararySender + " → " + tempararyAccepter + " (Value: '" + temparary + "')"); // 메시지에 실제 값 포함
    }
     ...
     */
    public void compToReg(String compiled, int memoryIndex, Register memory) {
        ...
        // 데이터 이동의 두 주체의 이름을 필드에 저장
        tempararySender = "Compiler"; // 송신은 항상 컴파일러
        tempararyAccepter = "Memory " + String.valueOf(memoryIndex);
        
        waitForEnter(tempararySender + " → " + tempararyAccepter + "(Value: '" + temparary + "')"); // 메시지에 실제 값 포함
    }

    /*
     * 제어장치에서 Mar로 index 데이터를 넘김
     * controlunit -> mar
     */
    public void valueToReg(String value, Register target) {
        ...
        // 데이터 이동의 두 주체의 이름을 필드에 저장
        tempararySender = "ControlUnit";
        tempararyAccepter = "MemoryAddressRegister";
        
        waitForEnter(tempararySender + " → " + "MemoryAddressBuffer" + " (Value: '" + temparary + "')");
    }
    ...
    public void readFromMemory(Register[] memory, MemoryAddressRegister mar, MemoryBufferRegister mbr) {
        ...
        // 데이터 이동의 두 주체의 이름을 필드에 저장
        tempararySender = "Memory " + String.valueOf(address);
        tempararyAccepter = "MemoryBufferRegister";
    ...
    public void writeToMemory(Register[] memory, MemoryAddressRegister mar, MemoryBufferRegister mbr) {
        ...
        waitForEnter("--- MEMORY WRITE COMPLETE ---");
    }
}
